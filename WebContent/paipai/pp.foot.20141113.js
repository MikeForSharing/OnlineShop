...
else{obj["on"+type]=handle;}};function $addToken(url,type,skey){var token=$getToken(skey);if(url==""||(url.indexOf("://")<0?location.href:url).indexOf("http")!=0){return url;}
...
else if(window.detachEvent){obj.detachEvent("on"+type,find(obj,type,handle)||handle);}};function $empty(){return function(){return true;}};function $expReturnCode(cfg){var codeType={success:1,error:2,logicError:3},_cfg={url:'',freq:1,uin:$getUin(),timeout:6000,timeoutCode:'444',formatUrl:true,isReport:false},sTime=new Date(),task,_guid=(new Date()).getTime();$extend(_cfg,cfg);function _report(code,type){if(_cfg.isReport){return;}
_cfg.isReport=true;var eTime=new Date(),domain=_cfg.url.replace(/^.*\/\//,'').replace(/\/.*/,''),cgi=encodeURIComponent(_cfg.formatUrl?_cfg.url.match(/^[\w|/|.|:|-]*/)[0]:_cfg.url),time=eTime-sTime,rate=_cfg.freq,uin=_cfg.uin,reportParam=['domain='+domain,'cgi='+cgi,'type='+type,'code='+code,'time='+time,'rate='+rate,'uin='+uin],reportUrl='http://c.isdspeed.qq.com/code.cgi?'+reportParam.join('&')+'&t='+Math.random();if(reportUrl&&Math.random()<(1/rate)){try{var _rName='_RETURNCODE'+_guid++;window[_rName]=new Image();window[_rName].src=reportUrl;}
catch(e){}}}
task=setTimeout(function(){_report(_cfg.timeoutCode,codeType.error);},_cfg.timeout);return{report:function(code,type){var _type;switch(type){case true:_type=codeType.success;break;case false:_type=codeType.error;break;default:_type=type;break;}
_report(code,_type);clearTimeout(task);},success:codeType.success,error:codeType.error,logicError:codeType.logicError}};function $extend(){var target=arguments[0]||{},i=1,length=arguments.length,options;if(typeof target!="object"&&typeof target!="function")
target={};for(;i<length;i++)
if((options=arguments[i])!=null)
for(var name in options){var copy=options[name];if(target===copy)
continue;if(copy!==undefined)
target[name]=copy;}
return target;};function $float(opt){var option={id:"",left:0,top:0,width:400,height:0,title:"",html:"",leaver:2,zindex:255,autoResize:false,opacity:0.5,cover:true,isCoverClose:false,dragble:false,fix:false,titleId:"",showClose:true,closeId:"",bgframeLeft:-2,bgframeTop:-2,cName:"module_box_normal vt_float",style:"stand",contentStyle:"",needOutCss:1,cssUrl:window.config_float_css||"http://static.paipaiimg.com/module/module_box.css",onInit:$empty(),onClose:$empty()};for(var i in opt){option[i]=opt[i];}
...
that.data?"":init(option.cssUrl,option.needOutCss);option.id=option.id?option.id:++that.data.zIndex;option.close=closeFloat;option.destruct=destructFloat;option.closeOther=closeOther;option.keepBoxFix=keepBoxFix;option.resize=resize;option.show=showBox;option.setPos=setPos;option.closeOther();option.show();that.data.list.push(option);if(option.dragble){$initDragItem({barDom:option.boxTitleHandle,targetDom:option.boxHandle});}
...
this.boxTitleHandle=$id(option.titleId||('float_title_'+this.id));this.boxCloseHandle=$id('float_closer_'+this.id);this.isCoverClose?(this.closeId+=',float_cover'):"";this.height?(this.boxHandle.style.height=(option.height=="auto"?option.height:option.height+"px")):"";this.width?(this.boxHandle.style.width=(option.width=="auto"?option.width:option.width+"px")):"";this.boxHandle.style.zIndex=that.data.zIndex;this.sw=parseInt(this.boxHandle.offsetWidth);this.sh=parseInt(this.boxHandle.offsetHeight);this.setPos();var _this=this;_this.boxCloseHandle?_this.boxCloseHandle.onclick=function(){_this.close();return false;}:"";if(_this.closeId){var arrClose=_this.closeId.split(",");for(var l=arrClose.length;l--;){var _el=$id(arrClose[l]);if(_el){_el.onclick=function(){_this.close();return false;};_el=null;}}}
...
function init(cssUrl,needOutCss){if(cssUrl&&needOutCss){$loadCss(cssUrl);}
that.data={};that.data.zIndex=option.zindex;that.data.list=[];createCover();that.data.showCover=showCover;that.data.closeCover=closeCover;function createCover(){var c=document.createElement("div");c.id="float_cover";c.style.display="none";c.style.width="0px";c.style.height="0px";c.style.backgroundColor="#cccccc";c.style.zIndex=250;c.style.position="fixed";c.style.hasLayout=-1;c.style.left="0px";c.style.top="0px";c.style.filter="alpha(opacity="+option.opacity*100+");";c.style.opacity=option.opacity;document.body.appendChild(c);if($isBrowser("ie6")){c.innerHTML='<iframe frameBorder="0" style="position:absolute;left:0;top:0;width:100%;z-index:-1;border:none;" id="float_cover_iframe"></iframe>';c.style.position="absolute";}
...
function closeCover(){that.data.cover.style.display="none";that.data.coverIsShow=false;clearInterval(that.data.coverTimer);}}};function $getContentHeight(){var bodyCath=document.body;var doeCath=document.compatMode=='BackCompat'?bodyCath:document.documentElement;return(window.MessageEvent&&navigator.userAgent.toLowerCase().indexOf('firefox')==-1)?bodyCath.scrollHeight:doeCath.scrollHeight;};function $getContentWidth(){var bodyCath=document.body;var doeCath=document.compatMode=='BackCompat'?bodyCath:document.documentElement;return(window.MessageEvent&&navigator.userAgent.toLowerCase().indexOf('firefox')==-1)?bodyCath.scrollWidth:doeCath.scrollWidth;};function $getCookie(name){var reg=new RegExp("(^| )"+name+"(?:=([^;]*))?(;|$)"),val=document.cookie.match(reg);return val?(val[2]?unescape(val[2]):""):null;};function $getFlashHtml(flashArguments,requiredVersion,flashPlayerCID){var _attrs=[],_params=[];for(var k in flashArguments)switch(k){case"noSrc":case"movie":continue;break;case"id":case"name":case"width":case"height":case"style":if(typeof flashArguments[k]!="undefined")_attrs.push(" ",k,'="',flashArguments[k],'"');break;case"src":if($isBrowser("ie"))_params.push('<param name="movie" value="',flashArguments.noSrc?"":flashArguments[k],'"/>');else _attrs.push(' data="',flashArguments.noSrc?"":flashArguments[k],'"');break;default:_params.push('<param name="',k,'" value="',flashArguments[k],'" />')}
if($isBrowser("ie"))if($isBrowser("ie10"))_attrs.push(' classid="clsid:',flashPlayerCID||"D27CDB6E-AE6D-11cf-96B8-444553540000",'"',' data="',flashArguments.src||"",'"');else _attrs.push(' classid="clsid:',flashPlayerCID||"D27CDB6E-AE6D-11cf-96B8-444553540000",'"');else _attrs.push(' type="application/x-shockwave-flash"');if(requiredVersion){_attrs.push(' codeBase="http://fpdownload.macromedia.com/get/flashplayer/current/swflash.cab#version=',requiredVersion,'"')}
return"<object"+_attrs.join("")+">"+_params.join("")+"</object>"};function $getMousePosition(e){var e=window.event?window.event:e;if(e.evt)e=e.evt;var pos=[];if(typeof e.pageX!="undefined"){pos=[e.pageX,e.pageY];}else if(typeof e.clientX!="undefined"){pos=[e.clientX+$getScrollPosition()[0],e.clientY+$getScrollPosition()[1]];}
return pos;};function $getPageScrollHeight(){return document.body.scrollTop?document.body.scrollTop:document.documentElement.scrollTop;};function $getPageScrollWidth(){var bodyCath=document.body;var doeCath=document.compatMode=='BackCompat'?bodyCath:document.documentElement;return(window.MessageEvent&&navigator.userAgent.toLowerCase().indexOf('firefox')==-1)?bodyCath.scrollLeft:doeCath.scrollLeft;};function $getQuery(name,url){var u=arguments[1]||window.location.search,reg=new RegExp("(^|&)"+name+"=([^&]*)(&|$)"),r=u.substr(u.indexOf("\?")+1).match(reg);return r!=null?r[2]:"";};function $getScrollPosition(){var _docElement=document.documentElement,_body=document.body,scrollLeft=(_docElement&&_docElement.scrollLeft)||(_body&&_body.scrollLeft)||window.pageXOffset||0,scrollTop=(_docElement&&_docElement.scrollTop)||(_body&&_body.scrollTop)||window.pageYOffset||0;return[scrollLeft,scrollTop];};function $getTarget(e,parent,tag){var e=window.event||e,tar=e.srcElement||e.target;if(parent&&tag&&tar.nodeName.toLowerCase()!=tag){while(tar=tar.parentNode){if(tar==parent||tar==document.body||tar==document){return null;}else if(tar.nodeName.toLowerCase()==tag){break;}}};return tar;};function $getToken(skey){var skey=skey?skey:$getCookie("skey");return skey?$time33(skey):"";};function $getUin(){var uin=$getCookie("uin")||$getCookie('uin_cookie')||$getCookie('pt2gguin')||$getCookie('o_cookie')||$getCookie('luin')||$getCookie('buy_uin');return uin?parseInt(uin.replace("o",""),10):"";};function $getWindowHeight(){var bodyCath=document.body;return(document.compatMode=='BackCompat'?bodyCath:document.documentElement).clientHeight;};function $getWindowWidth(){var bodyCath=document.body;return(document.compatMode=='BackCompat'?bodyCath:document.documentElement).clientWidth;};function $getY(e){var t=e.offsetTop||0;while(e=e.offsetParent){t+=e.offsetTop;}
return t;};function $getYP(e){var t=$getY(e),e=e.parentNode;while(0===t&&document.body!=e){t=$getY(e);e=e.parentNode;}
return t;};function $holdtitle(){(function(){var rememberTitle=document.title;try{document.attachEvent&&document.attachEvent('onpropertychange',function(){if(document.title!=rememberTitle){document.title=rememberTitle;}});}catch(e){}})();};function $id(id){return typeof(id)=="string"?document.getElementById(id):id;};function $inArray(t,arr){if(arr.indexOf){return arr.indexOf(t);}
...
return false;}else{return document.body?true:false;}};function $isLogin(){return($getCookie("p_skey")&&$getCookie("p_uin")||$getCookie("skey")&&$getCookie("uin"))?true:false;};function $itilReport(option){var opt={bid:"1",mid:"01",res:[],onBeforeReport:null,delay:5000}
for(var k in option){opt[k]=option[k];}
if(opt.res.length>0){window.reportWebInfo=function(json){};window.setTimeout(function(){opt.onBeforeReport&&opt.onBeforeReport(opt);var pstr=opt.bid+opt.mid+"-"+opt.res.join("|");var url="http://bases.wanggou.com/mcoss/webreport/ReportWebInfo?report="+pstr+"&t="+new Date()/1000;$loadUrl({url:url});},opt.delay);}};function $loadCss(path,callback){if(!path){return;}
...
el.onload=el.onreadystatechange=el.onerror=null;el.parentNode&&(el.parentNode.removeChild(el));el=null;}};(function(){var counter=1;$login=function(opts){var that=arguments.callee;var login={option:{'version':201309251107,'title':"拍拍-请您登录后继续刚才的操作",'containerId':"",'floatDialog':true,'modalDialog':true,'dragable':true,'showClose':true,'quickLogin':true,'checkLogin':true,'checkReady':true,'showProtocol':false,'site':'paipai','noChangeQQ':false,'defaultQQ':"",'type':"self",'action':"",'x':0,'y':0,'domain':"",'skin':"http://static.paipaiimg.com/module/module_box.css",'appid':'','onLogin':$empty(),'onReset':$empty(),'onClose':$empty(),'onResize':$empty(),'onSuccess':$empty(),'onFailure':$empty()},v:function(){return this.option.version;},init:function(opts){var option=this.option;for(var i in opts){option[i]=opts[i];}
...
option.show=this.show;option.close=this.close;option.resize=this.resize;option.doAction=this.doAction;option.syncLogin=this.syncLogin;option.counter=counter++;if(hn.indexOf("paipai.com")!=-1){$setCookie('returnurl','',-1,'/','paipai.com');$setCookie('referurl','',-1,'/','paipai.com');}
if(option.checkLogin&&$isLogin()){this.doAction();return;}
this.registerLoginEvent();this.load(option.skin);option.show(option);that.instance=option;return option;},syncLogin:function(){var hn=location.hostname,isQQ=hn.indexOf("qq.com")!=-1,isWG=hn.indexOf("wanggou.com")!=-1,isDone=false;var url,url2,url3,url4,url5;if(isQQ){url="http://ptlogin2.qq.com/ho_cross_domain?tourl="+encodeURIComponent("http://member.paipai.com/cgi-bin/ptlogin?returnurl=http://auction.paipai.com/null.shtml&loginfrom=20");url2="http://ptlogin2.qq.com/ho_cross_domain?tourl="+encodeURIComponent("http://member.wanggou.com/userlogin/ptlogin?returnurl=http://auction.paipai.com/null.shtml&loginfrom=20&daid=154");url3="http://ptlogin2.qq.com/ho_cross_domain?tourl="+encodeURIComponent("http://ecclogin.yixun.com/login/synclogin");url4="http://ptlogin2.qq.com/pt4_web_jump?succ_url="+encodeURIComponent("http://chong.qq.com")+"&daid=129&appid=677010801&pt4_token="+$getCookie("p_skey");url5="http://ptlogin2.qq.com/pt4_web_jump?succ_url="+encodeURIComponent("http://sudi.qq.com")+"&daid=189&appid=677010801&pt4_token="+$getCookie("p_skey");}else if(isWG){url="http://ptlogin2.wanggou.com/ho_cross_domain?tourl="+encodeURIComponent("http://member.paipai.com/cgi-bin/ptlogin?returnurl=http://auction.paipai.com/null.shtml&loginfrom=21");url2="http://ptlogin2.wanggou.com/ho_cross_domain?tourl="+encodeURIComponent("http://user.buy.qq.com/cgi-bin/ping/visitkey");url3="http://ptlogin2.wanggou.com/ho_cross_domain?tourl="+encodeURIComponent("http://ecclogin.yixun.com/login/synclogin");url4="http://ptlogin2.wanggou.com/ho_cross_domain?tourl="+encodeURIComponent("http://chong.qq.com");url5="http://ptlogin2.wanggou.com/ho_cross_domain?tourl="+encodeURIComponent("http://sudi.qq.com");}else{url="http://ptlogin2.paipai.com/ho_cross_domain?tourl="+encodeURIComponent("http://user.buy.qq.com/cgi-bin/ping/visitkey");url2="http://ptlogin2.paipai.com/ho_cross_domain?tourl="+encodeURIComponent("http://member.wanggou.com/userlogin/ptlogin?returnurl=http://auction.paipai.com/null.shtml&loginfrom=18&daid=154");url3="http://ptlogin2.paipai.com/ho_cross_domain?tourl="+encodeURIComponent("http://ecclogin.yixun.com/login/synclogin");url4="http://ptlogin2.paipai.com/ho_cross_domain?tourl="+encodeURIComponent("http://chong.qq.com");url5="http://ptlogin2.paipai.com/ho_cross_domain?tourl="+encodeURIComponent("http://sudi.qq.com");}
var doAction=function(){if(!isDone){isDone=true;login.doAction();}
if(timer){clearTimeout(timer);timer=null;}};var timer=setTimeout(function(){doAction();},2000);var img=new Image(),img2=new Image(),img3=new Image(),img4=new Image(),img5=new Image();img.src=url;img2.src=url2;img3.src=url3;img4.src=url4;img5.src=url5;},data:{mapDaid:{'wanggou.com':154,'buy.qq.com':128,'shop.qq.com':127,'paipai.com':126,'sudi.qq.com':189,'kuyoo.cn':130,'chong.qq.com':129,'etg.qq.com':131,'wkd.qq.com':66,'weikeduo.qq.com':132,'victor.qq.com':133,'piao.qq.com':76,'go.qq.com':70,'gaopeng.qq.com':134,'gaopeng.com':135,'518.qq.com':152,'licai.qq.com':190,'51buy.com':68,'yixun.com':174}},tools:{getDaid:function(domain){var mapDaid=login.data.mapDaid;for(var key in mapDaid){if(domain.indexOf(key)!=-1){return mapDaid[key];}}
return-1;}},registerLoginEvent:function(){ptlogin2_onLogin=function(){return that.instance.onLogin()?true:false;};ptlogin2_onReset=function(){return that.instance.onReset()?true:false;};ptlogin2_onClose=function(){if(location.hostname.indexOf("qq.com")!=-1){ptlogin2_onSuccess();}
return that.instance.onClose()?true:false;};ptlogin2_onResize=function(w,h){var login=that.instance;w=parseInt(w);h=parseInt(h);if(!login.onResize(w,h)){return false;}
...
return true;};ptlogin2_onSuccess=function(){var hn=location.hostname,isWG=hn.indexOf("wanggou.com")!=-1,isPP=hn.indexOf("paipai.com")!=-1;if(isWG||isPP){$itilReport({bid:"4",mid:"01",res:["0:1"]});}
var login=that.instance;if(!login.onSuccess()){return false;}
login.close();login.syncLogin();return true;};ptlogin2_onFailure=function(err){var hn=location.hostname,isWG=hn.indexOf("wanggou.com")!=-1,isPP=hn.indexOf("paipai.com")!=-1;if(isWG||isPP){$itilReport({bid:"4",mid:"01",res:["0:0"]});}
var login=that.instance;if(!login.onFailure(err)){return false;}
if(err){alert("登录失败！可能的错误原因："+err);}
that(login);return true;};ptlogin2_frame_onLoad=function(){var login=that.instance;if(login.noChangeQQ){var _ifrm=$id('login_frame_'+login.counter);if(_ifrm){var _u=_ifrm.contentWindow&&_ifrm.contentWindow.document.getElementById('u');if(_u){_u.disabled=true;}}}};},doAction:function(){var action=this.option.action;switch(this.option.type){case"func":action();break;case"top":top.location.href=action;break;case"parent":parent.location.href=action;break;case"self":location.href=action;break;case"blank":window.open(action);break;}},show:function(option){var hn=location.hostname,isQQ=hn.indexOf("qq.com")!=-1,isWG=hn.indexOf("wanggou.com")!=-1;if(!this.appid){if(isWG){this.appid=677010801;}else if(hn.indexOf("buy.qq.com")>-1){if(hn.indexOf("seller.buy.qq.com")>-1){this.appid=703010802;}else{this.appid=677010801;}}else{this.appid=isQQ?8000210:17000101;}}
if(!this.daid){this.daid=login.tools.getDaid(hn);$setCookie("daid",this.daid,525600);}
if(this.daid==154||this.daid==128||this.daid==127){this.title="网购-请您登录后继续刚才的操作";}else if(this.daid==126){this.title="拍拍-请您登录后继续刚才的操作";}
var query={"hide_border":1,"style":23,"daid":this.daid,"pt_safe":1,"hide_title_bar":1,"hide_close_icon":1,"target":"self","no_verifyimg":1,"f_url":"loginerroralert","bgcolor":this.floatDialog?"f2faff":"eef5ff","link_target":"blank","uin":this.defaultQQ,"appid":this.appid,"t":Math.random()};if(!this.quickLogin){query['enable_qlogin']=0;}
if(isQQ){var url='https://ssl.xui.ptlogin2.qq.com/cgi-bin/xlogin?';query['s_url']=hn.indexOf("buy.qq.com")>-1?'http://buy.qq.com%2Fredirect.html':(/(chong\b|^518\b|^licai\b\.qq\.com)/.test(hn)?'http%3A%2F%2Fchong.qq.com%2Fmember%2Flogin_s.shtml':'http://imgcache.qq.com%2Fqqshop%2Fac%2Fportal%2Fredirect.html');}else if(isWG){var url='https://ssl.xui.ptlogin2.wanggou.com/cgi-bin/xlogin?';query['s_url']=encodeURIComponent('http://member.wanggou.com/userlogin/ptlogin?loginfrom=21&daid=154');}else{var url='https://ssl.xui.ptlogin2.paipai.com/cgi-bin/xlogin?';query['s_url']=encodeURIComponent('http://member.paipai.com/userlogin/ptlogin?loginfrom=18');}
...
outTimer=setTimeout(function(){img.onload=img.onerror=$empty();$report(reportUrl+"&1=5000");},5000);var sTime=new Date();img.src=src+(src.indexOf("?")>-1?"&t=":"?t=")+Math.random();}}}}})();(function(){var isReady=false,readyList=[],timer;$ready=function(fn){if(isReady)
fn.call(document);else
readyList.push(function(){return fn.call(this);});return this;}
var onDOMReady=function(){for(var i=0,len=readyList.length;i<len;i++){readyList[i].apply(document);}
readyList=null;}
...
tar=tar.parentNode;}};var objMsg=window.AIOPAGE?{p:"aio.com"}:{p:"paipai.com"};if(document.domain=="pinpai.yixun.com"){objMsg.witem="1";}else if(document.domain=="witem.yixun.com"){objMsg.witem="2";}
pp&&(objMsg.ptag=pp);wg&&(objMsg.wgtag=wg);id&&(objMsg.id=id);name&&(objMsg.name=name);policy&&(objMsg.policy=policy);stuff&&(objMsg.stuff=stuff);material&&(objMsg.material=material);try{ECC.cloud.report.trace(objMsg);}
catch(e){}}});var loadPvCount=function(handle,type){if(type=='img'){handle=makeSendInfo(handle);};handle();};var makeSendInfo=function(_url){return function(){$report(_url);};};var paipai_pv={req_url:null,req_refer:null,req_host:null,req_host_ori:null,req_path:null,req_query:null,req_prot:null,doc:null,old_pageid:0,old_domainid:0,old_linkid:0,new_pageid:0,new_domainid:0,new_linkid:0,se_pageid:0,se_domainid:0,se_linkid:0,sc:'',url_encode:function(url)
...
{this.sc+=(this.sc==''?'':'^')+escape(cookieName)+';'+encodeURIComponent(cookieValue)+';'+(expires?parseInt(expires.getTime()/1000):'')+';';document.cookie=escape(cookieName)+'='+escape(cookieValue)+(expires?'; expires='+expires.toGMTString():'')+(path?'; path='+path:'')+(domain?'; domain='+domain:'')+(secure?'; secure':'');if(this.req_host.indexOf("qq.paipai.com")>=0)
...
pprd_p="";var regs={EA:/EA\.(\d+)\.(\d+)\.(\d+)(\D?)/gim,IA:/IA\.(\d+)\.(\d+)\.(\d+)(\D?)/gim,CT:/CT\.(\d+)\.(\d+)\.(\d+)(\D?)/gim,PD:/PD\.(\d+)\.(\d+)\.(\d+)(\D?)/gim,SEM:/SEM\.([%a-zA-Z0-9]+)(\-?)/gim,QZSNS:/QZSNS\.([\d\_]+)(\D?)/gim,QZZTC:/QZZTC\.([\w\!\=\+\/]+)([^\w\!\=\+\/]?)/gim,SEAB:/SEAB\.(\d+)(\D?)/gim,QQX:/QQX\.(\w+)(\W?)/gim,DISCUZ:/DISCUZ\.(\w+)(\W?)/gim,QZGDT:/QZGDT\.([\w\!]+)([^\w\!]?)/gim,CTZTC:/CTZTC\.([^&#\-]*)(&|$|#|\-)/gim,MART:/MART\.([^&#\-]*)(&|$|#|\-)/gim,CPS:/CPS\.([^&#\-]*)(&|$|#|\-)/gi,FOCUS:/FOCUS\.([^&#\-]*)(&|$|#|\-)/gi,CPC:/CPC\.([^&#\-]*)(&|$|#|\-)/gim,SC:/SC\.([\d,]+)([^\d,]?)/gim,QT:/QT\.([\w\!\_\$\|]+)(\W?)/gim,VB2CTAG:/VB2CTAG\.([\w\!\_\$\|]+)(\W?)/gim,JDPOP:/JDPOP\.([^\.\-]+)()/gim};var reg=regs[prefix];if(!reg.test(pprd_p))
...
this.do_set_cookie("PPRD_P",pprd_p,new Date((new Date()).getTime()+259200000),"/","paipai.com",0);}catch(e)
...
loadPvCount(function(){$report("http://service.paipai.com/cgi-bin/ping?rand="+Math.round(Math.random()*100000)+"&"+url_data);try{ECC.cloud.report.pv(objMsg);}
catch(e){}});}
...
{}},handle_jdpop_flag:function(){try{var reg=/[\?&#]jd_pop=([^\.&#]+)/gim;var arr=reg.exec(this.req_url);if(null!=arr&&arr.length==2)
{this.set_pprd_p_cookie('JDPOP',arr[1].replace(/-/g,''));}}
catch(e){}},handle_crm_report:function(){var curHref=window.location.href;var ptagArray=['20036.58.2','20036.58.3','20029.9.12','20200.4.2'];var ptag=$getQuery("PTAG",curHref);if($inArray(ptag,ptagArray)>=0){var reportCgi="http://my.paipai.com/crm/u/crmclick.xhtml",uin=$getQuery("uin",curHref),buin=$getQuery("buin",curHref),mktid=$getQuery("mktid",curHref),did=$getQuery("did",curHref),reportUrl;if(/^\d+$/.test(uin)&&/^\d+$/.test(buin)&&/^\d+$/.test(mktid)){if(did){reportUrl=reportCgi+"?uin="+uin+"&buin="+buin+"&mktid="+mktid+"&did="+did+"&t="+Math.random();}else{reportUrl=reportCgi+"?uin="+uin+"&buin="+buin+"&mktid="+mktid+"&t="+Math.random();}
$loadScript(reportUrl);}}
var crmsl=$getQuery("crmsl");crmsl&&$report("http://my.paipai.com/crm/u/itemclick.xhtml?crmsl="+crmsl);},main:function()
{try
{if(this.get_pars()!=0)
return;this.handle_old_rd();this.handle_new_rd();this.handle_se_refer();this.handle_speed_report();this.handle_qzone_src();this.handle_qzone_ztc();this.handle_qzone_gdt();this.handle_paipai_ct();this.handle_paipai_mart();this.handle_paipai_cpc();this.handle_paipai_cps();this.handle_qqshow_flag();this.handle_discuz_flag();this.handle_shopcoupon();this.handle_paipai_focus();this.handle_qqtuan_flag();this.handle_jdpop_flag();this.handle_vb2ctag_flag();this.handle_ping_report();this.handle_crm_report();}
catch(e)
{}}};paipai_pv.main();(function(){try{var vk=$getCookie('visitkey'),domainReg=/^(.+\.|)(wanggou\.com|qq\.com|paipai\.com)$/i,hostName=window.location.hostname.toLowerCase(),returnCode;if(!domainReg.test(hostName)){return;}
returnCode=$expReturnCode({url:'http://x.jd.com/jdpaipai?callback=onJdCookiesMapping',formatUrl:false});window['onJdCookiesMapping']=function(obj){try{if(obj.errCode=='0'){returnCode.report(obj.errCode,true);}
else{returnCode.report(obj.errCode,false);}}
catch(e){}}
$loadScript('http://x.jd.com/jdpaipai?callback=onJdCookiesMapping&vk='+vk+'&t='+Math.random());}
catch(e){}})();(function(){try{var vk=$getCookie('visitkey'),uin=$getUin(),pprd=$getCookie('PPRD_P'),cmqq=0,cmvk=0,cmqqReg=/CMQQ\.(\d+)_/,cmvkReg=/CMVK\.(\d+)_/,domainReg=/^(.+\.|)(wanggou\.com|paipai\.com)$/i,hostName=window.location.hostname.toLowerCase(),returnCode;if(!domainReg.test(hostName)){return;}
if(cmqqReg.test(pprd)){cmqq=cmqqReg.exec(pprd)[1];}
if(cmvkReg.test(pprd)){cmvk=cmvkReg.exec(pprd)[1];}
if((!uin&&vk==cmvk)||(uin==cmqq)){return;}
returnCode=$expReturnCode({url:'http://cm.e.qq.com/cm.fcg?a=1750934018',formatUrl:false});window['onGdtCookiesMapping']=function(obj){try{if(obj.errCode=='0'){returnCode.report(obj.errCode,true);}
else{returnCode.report(obj.errCode,false);}}
catch(e){}}
$loadScript('http://cm.e.qq.com/cm.fcg?a=1750934018&j=xyz&time='+Math.floor(new Date().getTime()/1000));}
catch(e){}})();function do_ping()
...
window.imTalk=function(uin,tid,sigT,sigP){var url=(tid)?("tencent://message/?uin="+uin+"&fromuserid="+tid+"&touserid="+tid+"&unionid=72000106&WebSiteName=拍拍网&Service=19&sigT="+sigT+"&sigU="+sigP):"tencent://message/?uin="+uin+"&fromuserid=no&touserid=no&unionid=72000106&WebSiteName=拍拍网&Service=19&sigT="+sigT+"&sigU="+sigP;var ua=window.navigator.userAgent.toLowerCase(),isAndroid=!!ua.match(/(android)\s+([\d.]+)/),isIos=!!ua.match(/(ipad|iphone|ipod).*os\s([\d_]+)/);if(isAndroid||isIos){$float({title:"提示",html:'噢，当前浏览器无法发起会话。如需联系卖家，请使用电脑或安装<a href="http://act.m.buy.qq.com/pc/pcSpread.xhtml?gcfa=20810021&_lp=1" target="_blank">QQ网购APP</a>联系',fix:true,width:'auto',height:100,cover:false});return false;}
if(ua.indexOf("msie")!=-1){try{window.location.href=url;$countRd('1050.1.1');}catch(e){$countRd('1050.1.2');showError(3);}}else if(/(firefox|safari|opera|chrome)/i.test(ua)||window.opera){window.location.href=url;$countRd('1050.1.5');}else{$countRd('1050.1.4');showError(1);}
...
$holdtitle();(function(win,doc){if($isBrowser("ie6")||!(doc.domain.indexOf("wanggou.com")>-1)){return;}
$ready(function(){var zoomDom=doc.createElement("div"),zoomTips=$id("zoomTips"),neverShowZoomTip=!!$getCookie("noZoom"),isIE=navigator.userAgent.toLowerCase().indexOf("msie")!=-1,hasReportZoom=false,hasReportCloseZoom=false,zoomReport=function(ret,errid){var retCode=$returnCode({url:"http://www.wanggou.com/zoomDetect",frequence:1});retCode.report(ret,errid);};zoomDom.style.cssText="position: absolute; right: 0px; bottom: 0px; visibility: visible;";zoomDom.innerHTML=$getFlashHtml({"src":"http://static.paipaiimg.com/flash/release/detectZoom/zoom.swf","width":"10","height":"10","allowScriptAccess":"always","id":"accessory_zoom","name":"zoom_detect","wmode":"transparent","scale":"noScale"});doc.body.appendChild(zoomDom);win.zoomDetectCallback=function(json){if(json.scale==1){return;}
if(!hasReportZoom){zoomReport(true,neverShowZoomTip?0:1);hasReportZoom=true;}
if(neverShowZoomTip){return;}
$id("zoomState").innerHTML=json.scale>1?"放大":"缩小";zoomTips.style.display="";}
$addEvent($id("neverShowZoomTip"),"click",function(e){zoomTips.style.display="none";$setCookie("noZoom",1,525600,"/","wanggou.com");});$addEvent($id("zoomClose"),"click",function(e){zoomTips.style.display="none";});$addEvent(doc,"keydown",function(e){if(e.ctrlKey&&(e.keyCode==48||e.keyCode==96)){zoomTips.style.display="none";if(!hasReportCloseZoom){zoomReport(false,neverShowZoomTip?2:3);hasReportCloseZoom=false;}}});});})(window,document);(function(win,doc){if(doc.domain.indexOf("wanggou.com")==-1){return;}
var totop=$id('wgGoToTop'),suggest=$id('wg_suggest'),floating=$id('wgfloating'),isie6=$isBrowser('ie6'),wh=$getWindowHeight?$getWindowHeight()-65:590,h=0;if(!totop||!suggest){return;}
setInterval(function(){var _h=$getPageScrollHeight();if(h==_h){return};h=_h;totop.style.bottom=h>300?'15px':'-65px';if(isie6){if(floating)floating.style.top=h+300+"px";suggest.style.top=h+wh-41+"px";totop.style.top=h+wh+"px";totop.style.display=h>300?'':'none';}},300);})(window,document);(function(win,doc){$addEvent($id("imgKnet"),"click",function(){var urlknet=$id("urlKnet"),str=urlknet.href;var str1=str.substring(0,(str.length-6));str1+=CNNIC_RndNum(6);urlknet.href=str1;});function CNNIC_RndNum(k){var rnd="";for(var i=0;i<k;i++){rnd+=Math.floor(Math.random()*10);}
return rnd;}})();})();
window['PP.foot']='1023:20141113:20141119144839';
window['PP.foot.time'] && window['PP.foot.time'].push(new Date());/*  |xGv00|d719147b788ba55261d7285f18245dd3 */